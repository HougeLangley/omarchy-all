...
# login.sh - Login and display manager configuration
...
case "$DISTRO" in
    arch)
        ...
        ;;
    debian)
        # Check if uwsm and plymouth are available
        if ! command -v uwsm &>/dev/null; then
            echo "Installing UWSM for Debian..."
            # Note: UWSM may need to be installed from source or alternative repository on Debian
            echo "Note: UWSM installation on Debian may require manual steps"
        fi
        
        if ! command -v plymouth &>/dev/null; then
            echo "Installing Plymouth for Debian..."
            sudo apt update && sudo apt install -y plymouth plymouth-themes
        fi
        ;;
    *)
        echo "Unsupported distribution for login configuration"
        exit 1
        ;;
esac
...
# Check if plymouth is installed before proceeding
if command -v plymouth &>/dev/null; then
    case "$DISTRO" in
        arch)
            ...
            ;;
        debian)
            # Debian uses update-initramfs instead of mkinitcpio
    ...
    if [ -d "/boot/loader/entries" ]; then # systemd-boot
        ...
        if ! grep -q "GRUB_CMDLINE_LINUX_DEFAULT.*splash" /etc/default/grub; then
            ...
            # Regenerate grub config based on distribution
            case "$DISTRO" in
                arch)
    ...
    if [ "$(plymouth-set-default-theme 2>/dev/null)" != "omarchy" ]; then
        ...
        sudo plymouth-set-default-theme -R omarchy 2>/dev/null || echo "Note: Plymouth theme setup may require manual configuration on Debian"
    fi
else
    echo "Plymouth not available, skipping Plymouth configuration"
fi
...
CCODE
    ...
    if command -v gcc &>/dev/null; then
        ...
        echo "Warning: gcc not available, skipping seamless-login compilation"
        rm /tmp/seamless-login.c
    fi
fi
